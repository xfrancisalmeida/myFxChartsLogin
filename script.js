...
// Fetch Azure Static Web App auth info
fetch('/.auth/me')
  .then(response => {
  ...
  .then(data => {
    ...
    // If user is authenticated, hide auth container & show main content
    if (data.clientPrincipal) {
      ...
      // Initialize the charts after successful auth
      initCSVData();
    } else {
...
/***************************************************************
2) FOREX DASHBOARD SCRIPT
   (Same charting code from your original modular approach)
***************************************************************/
...
// Helper: Split a CSV line using comma or tab delimiter (with BOM removal)
...
// Auto-load CSV data from OHLC_Snapshot.csv using fetch as Blob and FileReader
  ...
  fetch('OHLC_Snapshot.csv')
    ...
    .then(blob => {
      ...
      reader.onload = evt => {
        ...
        // Clean header values by removing non-printable chars
        csvHeader = splitLine(rawCSVLines[0]).map(h => h.trim().replace(/[^\x20-\x7E]/g, ''));
        console.log("CSV Header (fetch):", csvHeader);
...
// CSV Upload Handler (manual file selection, optional)
...
// On timeframe or timezone change, rebuild charts
...
document.addEventListener('click', function(e) {
  if (e.target && e.target.classList.contains('row-label')) {
    ...
    let rowCurrency = e.target.textContent.trim();
...
function findNewestCandleInEntireCSV() {
  ...
  for (let i = 1; i < rawCSVLines.length; i++) {
    ...
    if (realUTC > newestCandleUTCms) {
      newestCandleUTCms = realUTC;
    }
  }
}
...
function reParseAndRenderAll() {
  ...
  document.querySelectorAll(".chart-container").forEach(div => {
    ...
    let tf = div.dataset.timeframe;
    renderChart(div.id, sy, rc, tf);
  });
  updateInfoCards();
}
...
function renderChart(containerId, symbol, rowCurrency, timeframe) {
  ...
  if (!dataBySymbol[symbol] || !dataBySymbol[symbol][timeframe]) {
    Plotly.newPlot(containerId, [], {
      title: { text: symbol + " (" + timeframe + ") - No Data", x: 0.5, xanchor: 'center' },
  ...
  // If row currency does not match the symbol prefix, use pink
  if (rowCurrency.toUpperCase() !== "METALS" && !symbol.toUpperCase().startsWith(rowCurrency.toUpperCase())) {
    bgColor = pinkColor;
  }
...
function updateDataAgeCard() {
  ...
  let text = diffMin < 60
    ...
    : `${(diffMin / 60).toFixed(1)} hour${(diffMin / 60).toFixed(1) === "1.0" ? "" : "s"} old`;
...
function formatDDMonYYYY(dt) {
  ...
  return (dt.getUTCDate() < 10 ? "0" : "") + dt.getUTCDate() + "-"
